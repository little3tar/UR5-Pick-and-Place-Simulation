...
完整运行了一次仿真全流程，结果与之前一致：

| 步骤 | 结果 | 耗时 |
|---|---|---|
| Gazebo 仿真启动 (gzserver + gzclient) | ✅ | ~18s |
| UR5 机械臂 + Robotiq 控制器加载 | ✅ | — |
| 11 种乐高积木生成 (level 2) | ✅ | ~2s |
| YOLOv5 视觉识别 (CUDA, best.pt + depth.pt) | ✅ | 0.84s/帧 |
| **vision → motion_planning 管道** | ❌ | — |

> ❌ **崩溃原因确认**: `vision.py:429 rospy.signal_shutdown(0)` 在发布 `/lego_detections` 后关闭了整个 ROS 节点，导致 `motion_planning.py:367 wait_for_message` 收到 `rospy.exceptions.ROSInterruptException: rospy shutdown`，程序崩溃。根本原因是第四章记录的问题 1。
...
**影响范围**: 项目中 26 个 .py 文件用的是 `#!/usr/bin/env python`（全部附属于上游包），仿真中直接影响的只有 gazebo_ros_link_attacher/scripts/ 下的 7 个。
...
完全由坑 5 导致——后台 apt 持有锁，前台 apt 永远等待。用户手动在前台执行一条命令就成功了。
...
## 四、可通过修改仓库文件修复的问题（4 个）

| 文件 | 行号 | 问题 | 修复方法 |
|---|---|---|---|
| `vision/scripts/lego-vision.py` | 429 | `rospy.signal_shutdown(0)` 导致视觉处理一帧后就退出，motion_planning 永远等不到 `/lego_detections` | 改为持续运行（删除 shutdown，靠 rospy.spin 保持） |
| `motion_planning/scripts/motion_planning.py` | 367 | `get_legos_pos(vision=True)` 写死，必须等视觉发布数据才能运行 | 改成命令行参数 `--gazebo` 切换为从 Gazebo 模型位姿直接读取 |
| `gazebo_ros_link_attacher/scripts/*.py` (7个) | 1 | `#!/usr/bin/env python`，但 Ubuntu 20.04 没有 `python` 命令 | 统一改为 `#!/usr/bin/env python3` |
| `motion_planning/scripts/build_planning.py` | 78 | 缺少 `building.json` 配置文件，城堡搭建模式直接崩溃 | 添加文件存在性检查 + 清晰错误提示 |

### 可以通过新建文件避免的问题（1 个）

| 缺失 | 影响 | 修复方法 |
|---|---|---|
| 整个项目无 `requirements.txt` | 新用户不知道装哪些 Python 包 | 创建 requirements.txt，锁定 torch、numpy、matplotlib、ultralytics 等版本 |

---

## 五、无法通过修改仓库避免的（纯环境问题）

- **`/usr/bin/python` symlink** — Ubuntu 20.04 系统级差异，建议在 README 或 setup 脚本中说明
- **tzdata 交互式安装** — 所有大型 apt 安装都会遇到，建议提供一键 setup 脚本预设时区
- **catkin-tools osrf-pycommon** — apt 包依赖不完整，可通过 requirements.txt 间接覆盖
- **Python 版本冲突链** — 锁 `requirements.txt` 版本即可兜底

---

## 六、最终可用状态

模拟完全启动成功，所有基础组件已验证正常：

- ✅ Gazebo 仿真 + UR5 + 11种乐高
- ✅ Level 1-4 关卡系统
- ✅ YOLOv5 视觉识别（CUDA 加速，检测类型+朝向）
- ✅ 轨迹控制器（6 关节 + 夹爪）
- ✅ link_attacher 插件（attach/detach/setstatic）
- ❌ **完整抓取-分类** — IK 发散需修复
- ❌ **build_planning 城堡搭建** — 缺少 building.json
